...
const door=z.object({id:z.string(),name:z.string(),start:point,end:point,enabled:z.boolean(),type:z.enum(['hinged','double_hinged','sliding','double_sliding']),motionAxis:z.enum(['start','end']),motionDegrees:z.number().finite(),motionDirection:z.union([z.literal(-1),z.literal(1)]),plugin:z.string(),rightLeftRatio:z.number().positive()});
const floor=z.object({id:z.string(),name:z.string(),polygon:z.array(point).min(3),enabled:z.boolean(),textureName:z.string(),textureScale:z.number().positive(),textureRotation:z.number().finite(),ceilingTexture:z.string(),ceilingScale:z.number().positive(),indoor:z.boolean()});
const model=z.object({id:z.string(),name:z.string(),modelName:z.string(),x:z.number().finite(),y:z.number().finite(),yaw:z.number().finite(),z:z.number().finite(),static:z.boolean(),dispensable:z.boolean(),enabled:z.boolean()});
const measurement=z.object({id:z.string(),name:z.string(),start:point,end:point,distance:z.number().positive(),enabled:z.boolean()});
const building=z.object({config:z.object({buildingName:z.string(),referenceLevelName:z.string(),levelName:z.string(),elevation:z.number().finite()}),walls:z.array(wall),doors:z.array(door),floors:z.array(floor),models:z.array(model),measurements:z.array(measurement)}).optional();
export const AMRMapProjectFileSchema=z.object({format:z.literal('AMR_MAP_PROJECT'),version:z.literal('1.0'),project:z.object({id:z.string(),name:z.string(),metadata,image,objects:z.array(object),paths:z.array(path),zones:z.array(zone),robotConfigs:z.array(robot).min(1),building,createdAt:z.string(),updatedAt:z.string(),revision:z.number().int().nonnegative().default(1)})}).transform(file=>({
  ...file,
  project:{...file.project,building:file.project.building??{config:{buildingName:file.project.name,referenceLevelName:'L1',levelName:'L1',elevation:0},walls:[],doors:[],floors:[],models:[],measurements:[]}}
}));
export function migrateProject(value:unknown){const v=value as {version?:unknown};if(v?.version==='1.0')return AMRMapProjectFileSchema.parse(value);throw new Error(`Project file version ${String(v?.version??'unknown')} is unsupported.`)}
