          ...
          return <Group key={w.id}>
            ...
            {editable&&<>
              <Circle
                ...
                onMouseLeave={ev=>setHandleCursor(ev,'default')}
                onDragStart={ev=>{setHandleCursor(ev,'grabbing');p.commit()}}
                onDragMove={ev=>updateEndpoint('start',ev.target.x(),ev.target.y())}
                onDragEnd={ev=>{updateEndpoint('start',ev.target.x(),ev.target.y(),true);setHandleCursor(ev,'grab')}}
              ...
              <Circle
                ...
                onMouseLeave={ev=>setHandleCursor(ev,'default')}
                onDragStart={ev=>{setHandleCursor(ev,'grabbing');p.commit()}}
                onDragMove={ev=>updateEndpoint('end',ev.target.x(),ev.target.y())}
                onDragEnd={ev=>{updateEndpoint('end',ev.target.x(),ev.target.y(),true);setHandleCursor(ev,'grab')}}
                onClick={ev=>{ev.cancelBubble=true}}
                onTap={ev=>{ev.cancelBubble=true}}
              />
              {e.layers.labels.visible&&<>
                <Text
                  x={a.x+9/e.viewport.scale}
...
function PathDirectionOverlay({ path, metadata, scale, showBadge }: { path: NavigationPath; metadata: MapMetadata; scale: number; showBadge: boolean }) {
  ...
  //
  // The badge is rendered above the path. Keep the orientation arrow on the
  // opposite/lower side so the badge cannot cover it on horizontal segments.
  // A white halo keeps the arrow visible over occupancy pixels and path lines.
  const orientationArrows = orientation ? path.points.slice(0, -1).map((q, i) => {
    ...
    // Prefer the screen-down side of the segment normal because the badge
    // occupies the screen-up side of the path. Vertical segments simply use
    // one side of the line.
    const side = Math.abs(ny) > 1e-6 ? (ny >= 0 ? 1 : -1) : 1;
    const offset = 14 / scale;
    const half = Math.min(24 / scale, len * .24);
    ...
    return <Group key={`orientation-${orientation}-${i}`} listening={false}>
      <Arrow
        points={points}
